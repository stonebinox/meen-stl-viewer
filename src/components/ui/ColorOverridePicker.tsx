...
export function ColorOverridePicker() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={closePicker}>
      <div
        className="bg-ml-charcoal border border-white/[0.08] rounded-xl p-4 shadow-2xl flex flex-col gap-3"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <span className="text-sm font-serif text-ml-white truncate max-w-[180px]">{meshName}</span>
          <button onClick={closePicker} className="text-ml-silver hover:text-ml-white ml-4">✕</button>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-1.5 text-xs rounded bg-ml-gunmetal text-ml-platinum hover:bg-white/[0.1] transition-colors"
          >
            Reset mesh
          </button>
          <button
